...
当前 Cairn 采用"一个产品裁剪人 + 多个专职 agent"的协作方式，避免多个 agent 同时争抢主线。

| 角色                | 责任边界                                                            | 推荐分支                                    |
| ------------------- | ------------------------------------------------------------------- | ------------------------------------------- |
| 产品裁剪人（Owner） | 最终决定当前阶段做什么 / 不做什么；裁剪范围、排优先级、确认完成定义 | 不固定                                      |
| Codex（本地）       | 主工程实现、核心框架设计、模块边界、主要代码推进                    | `develop` 或从 `develop` 切出的短分支       |
| 白霓                | Desktop UI v0、交互体验、视觉系统、组件与页面原型                   | `feature/desktop-ui-v0`                     |
| 海棠                | QA / 集成守门、测试与 CI、文档对齐、架构边界检查、合并前风险清单    | `haitang/qa` 或从 `haitang/qa` 切出的短分支 |

协作规则：

1. Codex 负责"往前造"，但涉及产品边界时必须让产品裁剪人拍板。
2. 白霓负责"让它可被人理解和使用"，UI 变更必须附截图 / GIF 或可复现预览说明。
3. 海棠负责"别让它散架"，优先做验证、收口、风险提示；不抢主功能开发。
4. 所有 agent 都应主动辅助产品裁剪人给出利弊和建议，但最终范围决策以产品裁剪人为准。
5. 普通功能 / 修复 / 文档 PR 的 base 仍为 `develop`；不要直接 PR 到 `main`。
6. 长分支只承载角色边界，实际代码变更优先拆成短分支和小 PR。
