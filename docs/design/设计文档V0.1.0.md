# 多 Agent 协作工作台设计文档 V0.1.0

> 状态：内部主稿 / 正式产品重设计版
> 日期：2026-05-13
> 定位语义更新：2026-05-18 — 补充 runtime 控制面、证据层、非 Codex/Claude 复刻边界。
...
> **把项目从“Web 优先的多 Agent 工作台设想”升级为“共享核心 + 双外壳 + 可本地运行 + 可远程扩展”的正式产品设计主稿，并把对外主语收口为 runtime 控制面、证据层与人类接管工作台，而不是复刻任何单一 coding agent。**
...
> **一个面向个人开发者与小技术团队的、自托管且本地优先的 AI 工程工作台：连接 Codex / Claude Code / 本地模型等 runtime，以桌面端和 Web 端双外壳承载同一套协作核心，把复杂工程任务放大为可追踪、可接管、可验证、可复用的 run / artifact / trace 系统。**
...
- 一个 **桌面端与 Web 端共享同一运行语义的 operator workbench**
- 一个 **以 task / run / artifact / trace / operator intervention 为核心的 evidence layer**
...
- 单一聊天 bot
- 单一 IDE 内补全型 copilot
- 不做 Codex / Claude Code / Cursor / Cline / Aider 的替代品或复刻版
...
Runtime 负责“生成与执行”，Cairn 负责“组织、观察、接管、验证、沉淀与复用”。Cairn 不试图比 Codex / Claude Code 更会写代码；它把这些 runtime 的一次性执行变成有状态、有证据、有恢复语义、能被人类接管和复用的工程 run。
...
4. **Runtime Gateway Layer**
   ...
   - third-party endpoint boundary notices

5. **Infrastructure Layer**
   - DB / persistence
   - artifact store
   - code context index store
...
V0.1.0 建议把 **TypeScript / Node.js** 作为主产品语言与默认工程语言。
...
Cairn 需要一个本地优先的轻量代码上下文索引，用于把用户授权的代码目录转换为可查询、可追溯、可复用的 ContextPack。

边界：

- 自研实现，不引入 GitNexus 等许可证受限代码或包
- 默认遵守 `.gitignore`、隐私排除规则与用户 include / exclude 配置
- 索引是 Workspace 内的派生数据，可删除、可重建
- ContextPack 作为 Artifact 保留，并可被 Task 的 `context_refs` 引用
- R1 只做文件清单、文本搜索、TypeScript / JavaScript 基础 symbol outline、import/export 依赖边与 ContextPack artifact

详细设计见 [`code-context-index.md`](code-context-index.md)。
...
- `runtime_gateway` 保留，但只代表执行总线和 adapter 边界，不代表开放生态平台、模型托管平台或第三方 endpoint 背书层
...
### 11.4 Supervisor / Worker 是编排角色，不是内置模型身份

Supervisor 与 Worker 是 Cairn 内部的编排角色：它们描述谁负责规划、分发、综合与执行子任务，不表示 Cairn 内置一组独立模型 agent。具体生成和工具执行仍由 runtime adapter 调用 Codex CLI、Claude Code CLI、本地模型或其他用户配置 runtime 完成。

### 11.5 Supervisor 的职责阶段
...
AgentRun 是一次具体 runtime 调用的领域记录，不是 Cairn 自己创造的智能体身份。它必须能追到 runtime type、adapter capability、输入 artifact、输出 artifact 与 TraceEvent。
...
V0.1.0 不再使用“MVP”作为主叙事，但仍保留分 Release 的工程节奏。
...
- 轻量代码上下文索引（SourceRoot / ContextPack 基线）
- Chat / Runs / Tasks / Run Detail / Artifact / Trace 视图
- retry / rerun / cancel
- 基础 operator note
- Codex runtime 首发接入（OpenAI Codex CLI 子进程，用于验证 runtime 控制面与证据层链路，不表示绑定 OpenAI 或重做 Codex 产品体验）
...
- 更稳的观测与日志导出
- Generic OpenAI-Compatible Adapter（覆盖官方兼容接口、本地推理引擎与用户自配 endpoint；不内置、不推荐、不教学具体第三方转 API / 反代项目）
...
虽然 V0.1.0 已经能作为新主稿，但还有 5 个需要你尽快拍板的点：
...
5. **首发只接 Codex，还是同时把第二 runtime 留到 Release 2 明确承诺里**
   已收口原则：R1 Codex CLI 用于打通第一条 runtime 控制面链路；R2 Generic OpenAI-Compatible Adapter 是通用连接能力，不对任何具体第三方 endpoint 做专属适配或官方背书。
...
7. 从 **再做一个 multi-agent/coding-agent 项目** 收口为 **连接外部 runtime 的控制面、证据层与接管层**
...
> **这不是一个“先做 Web demo、以后补桌面端”的项目，也不是 Codex / Claude Code 的复刻版；它是一个以共享协作核心为中心、同时面向 Desktop 与 Web、同时兼顾本地工作区与远程工作区的 runtime 控制面、证据层与人类接管工作台。**
