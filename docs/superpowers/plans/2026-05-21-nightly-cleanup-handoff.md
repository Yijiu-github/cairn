...
2026-05-22 02:35 CST 复核：

- `git status --short`：仅剩本轮待处理的 internal-trial 归档/交接文档改动。
- `git diff --name-only`：本轮将只覆盖文档整理，不碰产品代码。
- 之前的 Desktop/Core/Runtime/UI-preview 主线改动已拆分为小提交。

当前已知未完成主线不在“泛化整理”，而在 internal trial 后续硬化：

- Desktop 自动化 window-level e2e 仍未补齐。
...
本轮文档整理提交 hash 将在提交后补写到本 handoff。
...
1. **Desktop window-level e2e 设计/最小自动化 spike**：围绕现有 Desktop internal-trial 路径，先确定可稳定自动化的 mock sidecar window-level smoke，不把真实 Codex 纳入默认 CI。
2. **真实 Codex 手动 smoke 复核**：按 runbook 再跑一条短任务，记录当前 Codex CLI / Node / OS 证据，只使用合成 prompt。
3. **Desktop renderer 主线小补强**：继续围绕 replay-loader / bounded payload preview / operator note 的错误态和空态补 targeted tests。
...
- 自动化 e2e 尚未覆盖 Desktop window-level internal-trial 路径；当前已有手动成功证据，但不能宣称自动化端到端完成。
