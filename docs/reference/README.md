...
| `external-project-radar.md` | 后续可阶段性参考的外部项目清单          | 🟡 Draft |
...
2. `external-project-radar.md` 是后续调研入口，只在对应阶段复查
3. `naming-conventions.md` 是 PR review 的快速依据
4. `postmortem-template.md` 在第一次事故后用，平时不打开
5. `meeting-notes/` 可选，但个人项目建议留 work log
...
- 外部项目参考只记录“何时看、借鉴什么、暂不采用什么”，不直接变成 Cairn 依赖
