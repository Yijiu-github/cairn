...
- Home 首屏已从内部观察面板调整为更面向用户的“派活工作台”：突出“派发给总 Agent”、Agent 统计、运行中的 Agent 与最近进展；任务草稿目前只保存在 renderer 本地，按钮仍触发 bounded internal-trial 路径，不是自由文本 Supervisor 执行入口。
- Home 首屏默认简中已将主要可见状态词收敛为“本地运行服务 / 运行安全 / 桌面桥接范围”等用户可读口径；底层仍是 Workspace Core dev sidecar bridge，不代表完整产品 UI 或真实自由文本派发已完成。
