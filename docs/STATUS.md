...
- 默认简中已将 Home 与第一轮 Run Detail / Artifact payload / Settings 会看到的主要状态词收敛为“本地运行服务 / 运行安全 / 回放证据 / 运行编号 / 桌面桥接范围”等用户可读口径；artifact payload 加载成功会提示“负载已加载、本地路径仍隐藏”，operator note 成功反馈会说明备注已记录并刷新回放证据 / 证据事件计数。底层仍是 Workspace Core dev sidecar bridge，不代表完整产品 UI 或真实自由文本派发已完成。
