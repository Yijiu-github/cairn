...
- ✅ Monorepo 工程基线、共享契约、领域持久化 schema、SQLite storage 与 Runtime Gateway 契约基线已启动
- 🚧 Desktop / Web / Workspace Core 应用代码尚未启动；当前主线是补齐 R1 的 application/core 基础与 Codex CLI adapter Spike
...
> 当前可运行的是工程校验与包级测试；应用开发服务会在 `apps/*` 启动后补齐。
